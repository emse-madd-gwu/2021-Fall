...
# Sum the values in f_x together to get the log-likelihoo
L <- sum(f_x)
L
```
...
L1 <- sum(dnorm(x, means[1], 4))
L2 <- sum(dnorm(x, means[2], 4))
L3 <- sum(dnorm(x, means[3], 4))
L4 <- sum(dnorm(x, means[4], 4))
L5 <- sum(dnorm(x, means[5], 4))
